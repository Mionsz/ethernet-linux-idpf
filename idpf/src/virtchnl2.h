...
/*
 * Compatibility layer for the Linux spellings this wire definition uses, so
 * that the definitions below stay byte-identical to the common header and can
 * be re-synced from it without a diff.  Kept self-contained: the integer types
 * come from compiler built-ins rather than a system header, because this file
 * is also consumed outside the kernel build.
 */
...
/*
 * Only virtchnl2.h needs these, and they stay outside the shared guard so
 * they are defined no matter which header established the type block.
 */

/* FreeBSD spells the address length ETHER_ADDR_LEN. */
#ifndef ETH_ALEN
#define ETH_ALEN 6
#endif

/* Size of a flex-array struct holding @n elements. */
#ifndef struct_size_t
#define struct_size_t(t, member, n)					\
	(__offsetof(t, member) + sizeof(((t *)0)->member[0]) * (n))
#endif

/* static_assert is not a keyword before C23; the assertions below use it. */
