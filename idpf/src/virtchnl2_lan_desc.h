 ...
 * that the definitions below stay byte-identical to the common header.  This
 * header is also included directly by idpf_txrx.h, so it carries its own copy
 * of the block; the shared guard makes whichever header is reached first the
 * one that defines it.
 */
