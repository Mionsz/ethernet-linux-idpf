...
void
idpf_test_wr32(struct idpf_hw *hw, u32 reg, u32 value)
{

	memcpy((uint8_t *)hw->mbx.vaddr + reg, &value, sizeof(value));
}

u32
idpf_test_rd32(struct idpf_hw *hw, u32 reg)
{
	u32 value;

	memcpy(&value, (uint8_t *)hw->mbx.vaddr + reg, sizeof(value));
...
void
idpf_acquire_lock(idpf_lock *lock __unused)
{
}

void
idpf_release_lock(idpf_lock *lock __unused)
{
}

void
idpf_destroy_lock(idpf_lock *lock __unused)
{
}

void *
idpf_calloc(struct idpf_hw *hw __unused, int count, size_t size)
{

	if (count <= 0 || size > SIZE_MAX / (size_t)count)
		return (NULL);

	return (idpf_test_kmalloc((size_t)count * size, M_ZERO));
}

void
idpf_free(struct idpf_hw *hw __unused, void *ptr)
{
...
void *
idpf_memset(void *addr, int value, size_t size,
    enum idpf_memset_type type __unused)
{
...
idpf_memcpy(void *dst, const void *src, size_t size,
    enum idpf_memcpy_type type __unused)
{
